...
export const DEFAULT_ADDRESS: Address = { port: 4600 }

const usable = (port: unknown): port is number =>
...
function coerce(held: unknown): Partial<Address> {
  ...
  const { port } = held as Record<string, unknown>

  return usable(port) ? { port } : {}
}
...
export async function writeAddress(wanted: { port: number }): Promise<Address> {
  if (!usable(wanted.port)) {
    throw new Error('A port must be a whole number between 1024 and 65535.')
  }

  const next: Address = { port: wanted.port }
